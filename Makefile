...
VERSION := $(shell cargo metadata --format-version 1 | jq -r '.packages[] | select(.name=="stackable-hive-operator") | .version')
...
docker-build:
	docker build --force-rm --build-arg VERSION=${VERSION} -t "docker.stackable.tech/stackable/hive-operator:${VERSION}" -f docker/Dockerfile .

docker-build-latest: docker-build
	docker tag "docker.stackable.tech/stackable/hive-operator:${VERSION}" \
...
version:
	yq eval -i '.version = strenv(VERSION) | .appVersion = strenv(VERSION)' /dev/stdin < deploy/helm/hive-operator/Chart.yaml
	yq eval -i '.version = strenv(DOCS_VERSION) | .prerelease = strenv(IS_NIGHTLY) != ""' /dev/stdin < docs/antora.yml

config:
	if [ -d "deploy/config-spec/" ]; then\
		mkdir -p deploy/helm/hive-operator/configs;\
		cp -r deploy/config-spec/* deploy/helm/hive-operator/configs;\
	fi

crds:
	mkdir -p deploy/helm/hive-operator/crds
	cargo run --bin stackable-hive-operator -- crd | yq eval '.metadata.annotations["helm.sh/resource-policy"]="keep"' - > deploy/helm/hive-operator/crds/crds.yaml

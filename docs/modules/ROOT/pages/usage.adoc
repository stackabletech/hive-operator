...
In this example, each Hive Metastore container in the `default` group will have a maximum of 2 gigabytes of memory. To be more precise, these memory limits apply to the container running the Hive Metastore but not to any sidecar containers that are part of the pod.

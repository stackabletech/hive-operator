...
RUN curl --fail -L https://repo1.maven.org/maven2/com/mysql/mysql-connector-j/8.0.31/mysql-connector-j-8.0.31.jar -o /tmp/mysql-connector-j-8.0.31.jar
----
...
spec:
  ...
  metastore:
    ...
    roleGroups:
      default:
        envOverrides:
          METASTORE_AUX_JARS_PATH: "/tmp/mysql-connector-j-8.0.31.jar"  # <2>
        replicas: 1
----

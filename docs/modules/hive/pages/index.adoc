...
Due to several reasons we do not feel that running Hive on Kubernetes makes much sense these days.
...
For this reason we decided to provide xref:trino:index.adoc[Trino] as a query engine in the Stackable Data Platform instead of Hive - which still uses the Hive Metastore, hence this operator exists.
